...
auto fun(){}
bool checkCycle(int node,int prnt,int l) {
    vis[node] = 1;
    dfsVis[node] = 1;
    for (auto it: g[node]) {
        if (!vis[it]) {
            if (checkCycle(it,node,l+1)) return true;
        } else if (dfsVis[it]&&it!=prnt&&!(l%2)) {
            return true;
        }
    }
    dfsVis[node] = 0;
...
void solve()
{
    ...
    ffor(i,1,n+1){
        ...
        if(g[i].sz==0||g[i].sz==1)continue;
        vfor(g[g[i][0]]){
            if(g[i][1]==*itr){pn(N);return;}
        }
    }
    vis.assign(n+1,0);
    dfsVis.assign(n+1,0);
