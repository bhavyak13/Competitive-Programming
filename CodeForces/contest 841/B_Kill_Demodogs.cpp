...
int modsub(int a, int b) {a = a % mod; b = b % mod; return (((a - b) % mod) + mod) % mod;}
...
{
    in(n);
    int x=modmul(modmul(n,(n+1)),((2*n)+1));
    x=modmul(x,modmulinv(3));
    int y=modmul(n,(n+1));
    y=modmul(y,modmulinv(2));
    pn((modsub(x,y)*2022)%mod);
}
