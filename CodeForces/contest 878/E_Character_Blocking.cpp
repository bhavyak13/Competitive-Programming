...
void solve()
{
    ...
    queue<ai>Q;
    while(q--){
        while(!Q.empty()&&Q.front()[0]==currTime){
            auto temp=Q.front();
            int indx=temp[1];
            int val=0;
            if(s[0][indx]!=s[1][indx])val=1;
        ...
        if(x==1){
            in(indx);
            indx--;
            seg.make_update(indx,0);
            Q.push({currTime+t,indx});
        }else if(x==2){
            in4(ts1,pos1,ts2,pos2);
            ts1--;
            ts2--;
            pos1--;
            pos2--;
            swap(s[ts1][pos1],s[ts2][pos2]);
            int val=0;
            if(s[0][pos1]!=s[1][pos1])val=1;
            seg.make_update(pos1,val);
            val=0;
            if(s[0][pos2]!=s[1][pos2])val=1;
            seg.make_update(pos2,val);
        }else{
            if(seg.make_query(0,n-1).val)pn(NO);
            else pn(Y);
        }
        currTime++;
    }
