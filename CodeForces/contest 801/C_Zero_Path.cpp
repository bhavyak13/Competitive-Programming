...
void solve()
{
    ...
    vvi minDP(n+1,vi(m+1,0));
    vvi maxDP(n+1,vi(m+1,0));
    // minDP[i] -> minimum possible sum of the values of the path covered till i,j starting from topleft
    // maxDP[i] -> maximum possible sum of the values of the path covered till i,j starting from topleft
    minDP[1][1]=maxDP[1][1]=a[1][1];
    ffor(i,2,n+1){
        minDP[i][1]=maxDP[i][1]=minDP[i-1][1]+a[i][1];
    }
    ffor(i,2,m+1){
        maxDP[1][i]=minDP[1][i]=maxDP[1][i-1]+a[1][i];
    }
    ffor(i,2,n+1){
        ffor(j,2,m+1){
            minDP[i][j]=min(minDP[i-1][j],minDP[i][j-1])+a[i][j];
            maxDP[i][j]=max(maxDP[i-1][j],maxDP[i][j-1])+a[i][j];
        }
    }
    // if (n+m) is even, take any path but the sum cant be non zero. think about 2*2 grid
    if(minDP[n][m]>0||maxDP[n][m]<0||maxDP[n][m]%2){
        pn(N);
    }else pn(Y);
}
