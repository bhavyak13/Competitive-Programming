...
void solve()
{
    in(n);
    map<string,int>m;
    int bothCost=im,firstCost=im,secondCost=im;
    ffor(i,0,n){
        in(t);
        string s;
        cin>>s;
        if(s[0]=='1'){
            firstCost=min(firstCost,t);
        }
        if(s[1]=='1'){
            secondCost=min(secondCost,t);
        }
        if(s[0]=='1'&&s[1]=='1'){
            bothCost=min(bothCost,t);
        }
    }
    if(bothCost==im){
        if((firstCost==im)||(secondCost==im)){
            pn(-1);
            return;
        }
    }
    pn(min(bothCost,firstCost+secondCost));
}
