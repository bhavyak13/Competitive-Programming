...
minimum ek child hai mtlb jo bhi alg departments hongy unka atleast 1 unmatch hoga he
aur jinka atleast 1 unmatch hoga mtlb vo other department heads hai !

*/
vvi g;
vector<bool>HeadEdges;
...
void solve()
{
    ...
    HeadEdges.assign(v+1,false); // stores edges of head
    vector<bool>isHead(v+1,false);// tells whether i th is head or not

    // one of the head -> head (2)
    isHead[head]=T;

    //Head edges are marked true
    vfor(g[head]){
        HeadEdges[*itr]=T;
    }
    HeadEdges[head]=T;

    // check who all are heads
    vfor(g[head]){
        if(!fun(*itr)){
            isHead[*itr]=T;
        }
    ...
    // printing departments of each employee
    int indx=0;
    for(auto i:g){
        if(isHead[indx])pt(mpp[indx]);
        else{
            for(auto j:i){
                if(isHead[j]){pt(mpp[j]);break;}
            }
        }
        indx++;
    }
