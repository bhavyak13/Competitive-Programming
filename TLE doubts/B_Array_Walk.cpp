...
// dp[i][z][1] -> 
// maximum score you can achieve such that you stand at i th index
// such that your last move was left and
// you are left with exactly z left moves

int n,Z,K;
vi a;
vector<vvi>dp;

int fun(int i,int fl, int z){
    int movesDone=i+2*(Z-z);
    if(movesDone>K)return 0;
    else if(movesDone==K)return a[i];
    else if(dp[i][fl][z]!=-1)return dp[i][fl][z];
    int ans=a[i]+fun(i+1,0,z);
    if(z&&!fl)ans=max(ans,a[i]+fun(i-1,1,z-1));
    return dp[i][fl][z]=ans;
}
