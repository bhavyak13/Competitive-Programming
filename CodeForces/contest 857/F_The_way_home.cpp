...
void solve()
{
    ...
    cin>>a;
    vi price(n+1,0);
    ffor(i,1,n+1)price[i]=a[i-1];

    vector<vector<aii>>g(n+5,vector<aii>());
    ffor(i,0,m){
        in3(x,y,z);
        g[x].pb({y,z});
    }
    ...
    dp[1]=0;
    pq.push({0,1,money,0});
    // ans, node, remMoney, maxPrice
    while(!pq.empty()){
        auto x=pq.top();
        pq.pop();
        int ans=x[0],node=x[1],remMoney=x[2],maxPrice=x[3];
        if(dp[node]!=ans)continue;
        // if(node==n){pn(ans);return;}
        maxPrice=max(maxPrice,price[node]);
        for(auto i:g[node]){
            int wt=i[1],v=i[0];
            int cost=wt,tempRemMoney=remMoney;
            int extraAns=0;
            if(cost>remMoney){
                cost-=remMoney;
                extraAns=ceil((db)cost/(db)maxPrice);
                tempRemMoney=extraAns*maxPrice-cost;
            }else{
                tempRemMoney-=cost;
            }
            int tempAns=ans+extraAns;
            // pn(node);
            // pt4(tempAns,v,tempRemMoney,maxPrice);
            if(dp[v]>tempAns){
                dp[v]=tempAns;
                pq.push({tempAns,v,tempRemMoney,maxPrice});
            }
        }
    }
    // ffor(i,1,n+1)pt(dp[i]);
    // cout<<endl;
    if(dp[n]==1e18)pn(-1);
    else pn(dp[n]);
}
