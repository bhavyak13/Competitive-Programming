...
void solve()
{
    ...
    map<int,int>m;
    deque<int>q,q2;
    ffor(i,0,n){cin>>a[i];q2.pb(a[i]);m[a[i]]++;}
    if(n%2==1){pn(-1);return;}
    vfor(m){if(i->ss%2!=0){pn(-1);return;}}
    vp ans;
    vi v;
    int i=0;
    while(!q2.empty()){
