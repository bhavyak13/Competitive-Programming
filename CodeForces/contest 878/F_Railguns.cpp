...
void solve()
{
    ...
    n++;
    m++;
    vector<vector<set<int>>>a(n+5,vector<set<int>>(m+5,set<int>()));
    in(Q);
    ffor(q,0,Q){
        in3(t,d,coord);
        if(d==1){
    ...
    while(!pq.empty()){
        ...
        // i+1,j
        if(!a[i+1][j].count(nwTym)){
            pq.push({nwTym,i+1,j});
        }else{
            // 5,6,7,8,9
            int f=1;
            while(a[i+1][j].count(nwTym)){
                if(a[i][j].count(nwTym-1)){// is position p nwTym k samne nhi rehskte!
                    f=0;
                    break;
                }
                nwTym++;
            }
            if(f && !a[i][j].count(nwTym-1))pq.push({nwTym,i+1,j});
        }
