...
logic : we tend to choose the leaf nodes initially which has more depth
then for non-leaf nodes we'll check the contribution
and we check contribution by their (depth-subtreeSize)

subtreeSize is subtracted from contibution beacuse id this node is choose
first of all this node will be choose after we have choosed all subnodes of this nodes as all subnodes of this node will have more height than this node
reason is by chooosing this node, for every node under the tree of this node will get one less happiness than before as this is also industry node now..
...
void solve()
{
    ...
    dfs(1,-1,0);
    vi v;
    ffor(i,1,n+1)v.pb(depth[i]-subtreeSize[i]+1);
    dsort(v);
    pn((accumulate(v.begin(),v.begin()+k,(ll)0)));
}
