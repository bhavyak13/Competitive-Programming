...
int modmulinv(int a){return binExpo(a,mod-2);}


/*------------------------------------begin------------------------------------
trick : 
    you cant find minumum using O(1) operations 
    but you can find pre usin O(1) operations
...
bool fun(int mid){
    ...
    ffor(i,0,n){
        ffor(j,0,m){
            if(a[i][j]>=mid)x[i][j]=1;
        }
    }
    ffor(i,1,n+1){
        ffor(j,1,m+1){
            pre[i][j]=pre[i-1][j]+pre[i][j-1]-pre[i-1][j-1]+x[i-1][j-1];
        }
    }
    for (int i = 0; i+mid <= n; i++){
        for (int j = 0; j+mid <= m; j++){
            int p=i+mid,q=j+mid;
            int ans=pre[p][q]-pre[p-mid][q]-pre[p][q-mid]+pre[p-mid][q-mid];
            if(ans>=mid*mid)return T;
        }
    }
    return F;
}
