...
bool ok(int mid,int a,int b,int H){// if on mid th day, H height can be achieved?
    int h=mid*a;
    h-=(mid-1)*b;
    return h>=H;
}
...
int fun(int a,int b,int H){
    int l=0,r=1;
    while(!ok(r,a,b,H))r=r<<1;
    while(r-l>1){
        int mid=(l+r)>>1;
        if(ok(mid,a,b,H))r=mid;
        else l=mid;
    }
    return r;
}
...
int fun2(int a,int b){
    int diff=(a-b);
    int day=fun(a,b,l)-1;
    int lbb=(day*a)-((day-1)*b);
    int ubb=lbb+diff;
    if(l>lbb&&r<=ubb)return day+1;
    else return -1;
}
