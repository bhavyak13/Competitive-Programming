...
/*------------------------------------begin------------------------------------
what is question?
create a string in which 
freq of all characters are same let say f or 0
and minimize the s[i]-t[i];
*/
...
void solve()
{
    ...
    vector<ai>X;
    ffor(i,0,26)X.pb({freq[i],i});
    sort(all(freq));
    reverse(all(freq));
    dsort(X);
    int ans=im,val=-1;
    for(int i=1;i<=26;i++){
        // i -> number of distinct characters
        if(n%i)continue;
        int f=n/i;// frequency of each distinct character
        int c=0,cnt=0;
        ffor(j,0,i){
            if(X[j][0]>=f)continue;
            c+=f-X[j][0];
    ...
    pn(ans);
    int neededFreq=n/val;
    string t=s;
    vi tbcInd(n,1);// to be change indices
    vi color(26,0);
