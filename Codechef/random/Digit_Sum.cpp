...
// dp[numdig][tight] -> summation of (sum of digits) of all 'numDig' digit numbers, tight 
...
int cnt(string num,int numDig,int tight){
    if(!tight)return (pow(10,numDig)+0.1); 
    if(!numDig)return 1;
    int ans=0;
    int rdig=(num[num.sz-numDig]-'0');
    ffor(dig,0,rdig+1){
        ans+=cnt(num,numDig-1,(tight&(dig==rdig)));
    }
    return ans;
}

int fun(string num,int numdig,int tight){
    if(numdig<=0)return 0;
    if(dp[numdig][tight]!=-1)return dp[numdig][tight];
    int ldig=0;
    int rdig=tight?(num[num.sz-numdig]-'0'):9;
    int ans=0;
    ffor(dig,ldig,rdig+1){
        int x=dig*cnt(num,numdig-1,(tight&(dig==rdig)));
        int y=fun(num,numdig-1,(tight&(dig==rdig)));
        ans+=x+y;
    }
    return dp[numdig][tight]=ans;
}
