...
let say degree of node is odd then -> 
removing only that node will be sufficient for us :)

else (degree of node % 2 == 0)
then we'll see if it is directly connected to any other even degree node
if yes we will check that node 
removing these 2 vertices in turn lead to even edges :)
...
void solve()
{
    ...
    int ans=1e18;
    if(!(m&1))ans=0;// friend pairs -> odd
    // for odd degree nodes
    ffor(i,1,n+1)if(g[i].sz&1)ans=min(ans,a[i-1]);
    // for even:
    ffor(i,0,m){
        if(g[v[i][0]].sz%2==g[v[i][1]].sz%2)ans=min(ans,a[v[i][0]-1]+a[v[i][1]-1]);
    }
    pn(ans);
}
