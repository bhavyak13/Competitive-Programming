...
auto fun(int t){return (t*(t-1))/2;}

void solve()
{
    in2(players,teams);

    int eachValue=players/teams;
    int remaining=players-(eachValue*teams);
    int a=fun(eachValue+1)*(remaining);
    a+=fun(eachValue)*(teams-remaining);
    

    int x=players-teams+1;
    int b=fun(x);
    pt2(a,b);
}
