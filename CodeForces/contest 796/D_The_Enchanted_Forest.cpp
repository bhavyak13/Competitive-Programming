...
void solve()
{
    ...
    if(k<=n){
        ...
        pn(ans+(k*(k-1)/2));
        //added k-1*k/2 to ans bcz eaxh time we are going on next place
        //1 mushroom is being added to that stage also!...except for the very first stage!
        return;
    }
    //k>n
    ...
    /*
    for k>n the solution is for first (k-n) moves dont move from 1st position!
    ans for last n moves take all mushroms tht have grown in each i
    that is on each i there will be : 
    (k-n)
    +(mushrrom already present on ith position)
    +(moves taken to come from 1st mushroom to ith mushroom)
    therefore ans= (k-n)*n + sum(a[i]'s) + (n*(n-1)/2)
    */
    int ans=0;
    ffor(i,0,n)ans+=a[i];
    ans+=(k-n)*n;
