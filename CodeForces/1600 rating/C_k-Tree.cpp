...
{
    in3(n,k,d);
    // dp[i][0] -> number of ways to make total weight of i, such that we dont use any edge of weight d in it
    // dp[i][1] -> number of ways to make total weight of i, such that we use atleast one edge of weight d in it
    vvi dp(n+5,vi(2,0));
    dp[0][0]=1;
    ffor(i,1,n+1){
        ffor(wt,1,k+1){
            if(i-wt>=0){
                if(wt<d) dp[i][0]=modadd(dp[i][0],dp[i-wt][0]);
                else dp[i][1]=modadd(dp[i][1],dp[i-wt][0]);// you aldready have min. weight edge hence you can take no. of edges with (i-wt) who doest hve min. edge as well as i-wt edge who can have i-wt wt
                dp[i][1]=modadd(dp[i][1],dp[i-wt][1]);
            }
        }
    }
    pn(dp[n][1]);
