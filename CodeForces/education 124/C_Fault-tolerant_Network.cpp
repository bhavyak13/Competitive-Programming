...
void solve()
{
    ...
    int ans=0;
    int l1,r1,l2,r2;
    l1=l2=r1=r2=LONG_LONG_MAX;
    ffor(i,0,n){
        l1=min(l1,abs(a[0]-b[i]));
        l2=min(l2,abs(a[n-1]-b[i]));
        r1=min(r1,abs(a[i]-b[0]));
        r2=min(r2,abs(a[i]-b[n-1]));
    }
    ans=min(abs(a[0]-b[0]),l1+r1);
    ans+=min(abs(a[n-1]-b[n-1]),l2+r2);
    int x=min(abs(a[0]-b[n-1]),l1+r2)+min(abs(a[n-1]-b[0]),l2+r1);
    pn(min(x,ans));
}
