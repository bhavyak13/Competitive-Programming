...
{
    /*
    what happens if there is only one ball
    _ _ _ _ _
    observation: if max elemnt is M then there will be M-1 positions which cant be empty!(-1 bcz ek ded ko bhi deskta  1st ball ses he aur ball vps nhi aaegi pr!)
    -> if we can fill all the empty spaces then we can do it using just one ball!!
    ->else we'll require (Max Element - (total sum-(max elemnt-1))) no of balls + 1(initial one)!
    */
    in(n);
    vi a(n);
    int maxi=0,s=0,ans;
    ffor(i,0,n){cin>>a[i];maxi=max(maxi,a[i]);s+=a[i];}
    if(maxi>(s-maxi)){ans=maxi-(s-maxi);}
