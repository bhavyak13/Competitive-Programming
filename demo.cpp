...
// /*------------------------------------begin------------------------------------*/
// int ans=0;
// int fun(vector<string>grid,int i,int j,char prev){
//     pt2(i,j);
//     if(i>grid.size()||j>grid[i].size()){
//         return 0;
//     }
//     if(grid[i][j]=='.'){
//         return ans=max(ans,max(fun(grid,i+1,j,grid[i][j]),fun(grid,i,j+1,grid[i][j])));
//     }
//     return ans=max(ans,1+max(fun(grid,i+1,j,grid[i][j]),fun(grid,i,j+1,grid[i][j])));
// }
...
auto fun(vector<string>transitions,vector<string>queries){
    ...
    for (auto i =transitions.begin() ; i!=transitions.end(); i++){
        ...
        auto ok=true;
        for (int d = 0; d < x; d++){
            if(str[d]=='-'){ok=false;continue;}
            if(ok){s1.push_back(str[d]);}
            else{s2.push_back(str[d]);}
        }
        m[s1].push_back(s2);
        m[s2].push_back(s1);
    }
...
// auto fun2(string s1,string s2,map<string,vector<string>>m){
//     vector<string>ans;
//     ffor(m[s1]){
//         fun2()
//     }
