...
void solve()
{
    ...
    cin>>a;
    sort(all(a));
    int ans=0,i=0,bags=0;
    while(i<n){
        int e=a[i],tc=0,indx=i;
        while(indx<n&&a[indx]==e){// count bags for this state
            indx++;
            tc++;
        }
        if(i&&a[i]-a[i-1]>1){// if jump>1 new bags req = tc
            ans+=tc;
            bags=0;
        }else if(tc>bags){// if any new bag is required for jump==1
            ans+=tc-bags;
        }
        bags=tc;
        i=indx;
