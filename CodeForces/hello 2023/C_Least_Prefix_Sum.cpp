...
* * * * * (X  *) * * * * 
* * * * * (X  * *) * * * 
sbse zda negative -> convert positive m then good for us

m=3
-1 -2 -3 15 -2 -4 -8 -5 
-1 -2 (-3) 15 -2 -4 -8 -5 

tot 13-4=9-8=1-5=-4+16=12
op used -> -8 -> 8
ans += 16
...
i,x ka sum agr X s kam hua
then maximum element uthao aur use minus krdo
...
void solve()
{
    ...
    int op=0;
    if(n==1){pn(0);return;}
    // if first element is positive and all are positive then no need to negate it
    if(a[k]>0&&k){a[k]*=-1;op++;}
    //right side
