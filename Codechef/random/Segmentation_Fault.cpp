...
{
    // solution : 
    // for ith person to be theif
    // the range that ith person say must not include i
    // the range that all other person say must include i
    in(n);
    vp pre(n+1),suff(n+2),a(n+1);
    pre[0]=suff[n+1]={1,n};
    ffor(i,1,n+1){
        in2(x,y);
        a[i].ff=x;a[i].ss=y;
        pre[i].ff=max(pre[i-1].ff,x);
        pre[i].ss=min(pre[i-1].ss,y);
    }
    bfor(i,n+1,1){
        int x=a[i].ff;
        int y=a[i].ss;
        suff[i].ff=max(suff[i+1].ff,x);
        suff[i].ss=min(suff[i+1].ss,y);
    }
    vi ans;
    ffor(i,1,n+1){
        bool isIncluded=(a[i].ff<=i&&a[i].ss>=i);
        bool otherPersonIncludes=
        (pre[i-1].ff<=i&&pre[i-1].ss>=i)&&
        (suff[i+1].ff<=i&&suff[i+1].ss>=i);
        if(!isIncluded&&otherPersonIncludes)ans.pb(i);
    }
    pn(ans.sz);
    vfor(ans)pn(*itr);
}
