...
void solve()
{
    ...
    //dp[i] -> true if, we can form a sequence from prefix of i!
    ffor(i,0,n){
        //if length is written before the sequnce {length is written before sequnce}
        // (T) 3 1 2 3
        //     4 5 6 7
        dp[min(n+3,i+a[i])]=(dp[min(n+3,i+a[i])])|((i==0)?T:dp[i-1]);
        //right
        // 4 5 6 (3)
        // 0 1 2 3
        int indx=i-a[i]-1;
        if(indx==-1)dp[i]=T;
        else if(indx>=0)dp[i]=dp[i]|dp[indx];
    }
    pn((dp[n-1]?Y:N));
}
