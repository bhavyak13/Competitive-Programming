...
case 1 : _ _ _ _ R 
adding red flower to end
in this case dp[i]=dp[i-1]

case 2 : _ _ _ _ W
adding whitw flower at end
in this case last k-1 flowers should be white and remaining (i-k) flowers could be anything white/red
...
void solve()
{
    ...
    int N=100005;
    vi dp(N,1);
    // dp[i] -> number of ways to eat i flowers under given constraints;
    ffor(i,k,N){
        dp[i]=dp[i-1];
    ...
    vi pre(N,0);
    ffor(i,1,N)pre[i]=(pre[i-1]+dp[i])%mod;
    while(q--){
        in2(x,y);
        pn(((pre[y]-pre[x-1])+mod)%mod);
    }
}
